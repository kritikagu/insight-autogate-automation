...
test.describe('Universal Login passwordless login', () => {
  ...
  const homePageUrl = 'https://stg.autogate.co?gi=true';
  const existingEmail = 'went-myself@y3kbv132.mailosaur.net';
  const existingFirstName = 'mailosaur';
...
/*
  if (!homePageUrl) {
    throw new Error('Environment variables for login URL or username are not set.');
  }

  if (!resendCodeTimeout) {
    throw new Error('Environment variables for resend code timeout are not set.');
  }
...
*/
  test.beforeEach(async ({ page }) => {
   ...
   // await page.getByRole('link', { name: 'Sign in or log in' }).click();
  ...
  // --- Test commented out due to the OTP code expiration time being >5 minutes which is too long for the test ---
  ...
  test('User login and cancel', async ({ page, commonFixture, mailosaurFixture }) => {
    test.slow();
  ...
  test('User login resend code', async ({ page, mailosaurFixture, commonFixture }) => {
    test.slow(); // Increase timeout for this test due to the resend code delay

    await commonFixture.fillEmailAndProceed(page, existingEmail);

    // Resend code
    await page.waitForTimeout(resendCodeTimeout + 1000); // wait 30 seconds to be allowed to resend the code
    await page.getByRole('button', { name: 'Resend code', exact: false }).click();

    // Get new otp and fill it
    const message = await mailosaurFixture.getEmailByAddress(existingEmail, new Date(Date.now() - 10000)); // Get the most recent email
    const otp = await mailosaurFixture.getOTPFromEmail(message);
    await commonFixture.fillOTP(page, otp);
    await page.getByRole('button', { name: 'Confirm' }).click();
